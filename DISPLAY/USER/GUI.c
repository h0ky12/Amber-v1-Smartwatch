...
void LCD_DrawLine(u16 x1, u16 y1, u16 x2, u16 y2)
{
	...
	{  
		LCD_DrawPoint(uRow,uCol,0x0000);//����
		xerr+=delta_x ; 
		yerr+=delta_y ; 
		if(xerr>distance) 
		{ 
...
{  
	...
	}else//���ӷ�ʽ
	{
		for(pos=0;pos<size;pos++)
		{
		    ...
		    {   
				POINT_COLOR=fc;              
		        if(temp&0x80)LCD_DrawPoint(x+t,y+pos,0x0000);//��һ����
		        temp<<=1; 
		    }
		}
	}
	  ...
	  { 	LCD_SetWindows(x,y,x+16-1,y+16-1);
		    for(i=0;i<16*2;i++)
		    {
		    	...
		    	{	
					...
					{
						POINT_COLOR=fc;
						if(tfont16[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y,0x0000);//��һ����
						x++;
						if((x-x0)==16)
						{
							x=x0;
			...
			for (k=0;k<HZnum;k++) 
			{
			  ...
			  { 	LCD_SetWindows(x,y,x+24-1,y+24-1);
				    for(i=0;i<24*3;i++)
				    {
							...
							{
								POINT_COLOR=fc;
								if(tfont24[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y,0x0000);//��һ����
								x++;
								if((x-x0)==24)
								{
									x=x0;
			  ...
			  { 	LCD_SetWindows(x,y,x+32-1,y+32-1);
				    for(i=0;i<32*4;i++)
				    {
						for(j=0;j<8;j++)
				    	{
							...
							{
								POINT_COLOR=fc;
								if(tfont32[k].Msk[i]&(0x80>>j))	LCD_DrawPoint(x,y,0x0000);//��һ����
								x++;
								if((x-x0)==32)
								{
									x=x0;
